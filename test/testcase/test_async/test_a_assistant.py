...
from test.config import chat_model_id, sleep_time, text_model_id, nums
...
@allure.epic("test_async_assistant")
...
class TestAssistant(Base):
    ...
    async def test_a_create_assistant(self, a_create_assistant_data):
        ...
        #     logger.info(f'a_create_assistant_function_id:{Base.function_id}')
        #     assistant_dict["tools"][0]["id"] = Base.function_id
...
class TestChat(Base):
        ...
        @pytest.mark.asyncio
        async def test_a_create_chat(self, async_chat_num):
            # List chats.
            for x in range(10):
                old_res = await a_list_chats(assistant_id=self.assistant_id)
                old_nums = len(old_res)
                # Create a chat.
                metadata = {f"test{x+1}": f"test{x+1}"}
...
class TestMessage(Base):
    ...
    @pytest.mark.parametrize("async_message_num", nums)
    @pytest.mark.asyncio
    async def test_a_create_user_message(self, async_message_num):
        # List messages.
        old_res = await a_list_messages(assistant_id=self.assistant_id, chat_id=self.chat_id)
        old_nums = len(old_res)
        # Create a user message.
        text = f"hello test{async_message_num}"
    ...
    async def test_a_list_messages(self):
        ...
        after_res = await a_list_messages(limit=nums_limit, after=after_id, assistant_id=self.assistant_id, chat_id=self.chat_id)
        pytest.assume(len(after_res) == nums_limit)
        twice_nums_list = await a_list_messages(limit=nums_limit * 2, assistant_id=self.assistant_id, chat_id=self.chat_id)
        pytest.assume(len(twice_nums_list) == nums_limit * 2)
        pytest.assume(after_res[-1] == twice_nums_list[-1])
        pytest.assume(after_res[0] == twice_nums_list[nums_limit])
        before_id = after_res[0].message_id
        before_res = await a_list_messages(limit=nums_limit, before=before_id, assistant_id=self.assistant_id, chat_id=self.chat_id)
        pytest.assume(len(before_res) == nums_limit)
        pytest.assume(before_res[-1] == twice_nums_list[nums_limit - 1])
        pytest.assume(before_res[0] == twice_nums_list[0])
    ...
    @allure.story("test_a_generate_assistant_message_by_stream")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    ...
    @allure.story("test_a_generate_assistant_message_in_user_message_not_created")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    ...
    @allure.story("test_a_create_user_message_in_generating_assistant_message")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    @pytest.mark.abnormal
    ...
    @allure.story("test_a_generate_assistant_message_in_generating_assistant_message")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    @pytest.mark.abnormal
    ...
    @allure.story("test_a_generate_assistant_message_in_generated_assistant_message")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    @pytest.mark.abnormal
    ...
    @allure.story("test_a_generate_assistant_message_in_action_deleted_assistant")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    @pytest.mark.abnormal
    ...
    @allure.story("test_a_generate_assistant_message_in_collection_deleted_assistant")
    @pytest.mark.run(order=30)
    @pytest.mark.asyncio
    @pytest.mark.abnormal
